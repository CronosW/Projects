...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(0, 3.0f, 0));
		modelMatrix = glm::rotate(modelMatrix, RADIANS(45.0f), glm::vec3(0, 1, 0));
		RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
	}
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_O)) {
		this->projectionMatrix = glm::ortho(-4.0, 4.0, -4.0, 4.0, 0.1, 100.0);
	}
	if (window->KeyHold(GLFW_KEY_P)) {
		this->projectionMatrix = glm::perspective(RADIANS(100.0), 1.0, 0.1, 100.0);
	}

	if (window->KeyHold(GLFW_KEY_B)) {
		curent += 1;
		this->projectionMatrix = glm::perspective(RADIANS(curent), 1.0, 0.1, 100.0);
	}
	...
	if (window->KeyHold(GLFW_KEY_G)) {
		prim -= 1;
		sec += 1;
		this->projectionMatrix = glm::ortho(prim, sec, -4.0, 4.0, 0.1, 100.0);
	}
	if (window->KeyHold(GLFW_KEY_H)) {
		tert -= 1;
		cvadru += 1;
		this->projectionMatrix = glm::ortho(-4.0, 4.0, tert, cvadru, 0.1, 100.0);
	}
