...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0.7, 0.2, 0.3));
	}
...

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0.55, 0.5, 0.75));
	}
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	glUniform1i(is_spotlight, is_spotlight_);

	int cut_off = glGetUniformLocation(shader->program, "cut_off");
	glUniform1f(cut_off, cut_off_);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_UP)) {
			lightDirection = glm::rotate(glm::mat4(1), deltaTime * speed, glm::vec3(1, 0, 0)) * glm::vec4(lightDirection, 1.0f);
			speed++;
		}
		...
		if (window->KeyHold(GLFW_KEY_LEFT)) {
			lightDirection = glm::rotate(glm::mat4(1), deltaTime * speed, glm::vec3(0, 0, -1)) * glm::vec4(lightDirection, 1.0f);
			speed++;
		}
